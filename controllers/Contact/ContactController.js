...
module.exports = {
    ...
    try {
      ...
      if (response.success) {
        const transformedData =
          await new ContactTransformer().getTransformedData(req, response.data)
  ...
  get: async (req, res) => {
    try {
      // We only pass the body object, never the req object
      const response = await ContactServiceInstance.getById(
  ...
  update: async (req, res) => {
    try {
      ...
      const response = await ContactServiceInstance.update(
        req.params.contactId,
        contactData
      )
      if (response.success) {
  ...
  delete: async (req, res) => {
    try {
      // We only pass the body object, never the req object
      const response = await ContactServiceInstance.deleteById(
        req.params.contactId
      )
      if (response.success) {
